...
const App: React.FC = () => {
  ...
  const processImage = async () => {
    ...
    try {
      ...
      const orientationResponse = await ai.models.generateContent({
        model: GEMINI_MODEL_NAME,
        contents: { parts: [{ inlineData: { mimeType: imageMimeType, data: imageData } }, { text: orientationPrompt }] },
      });
      
      const orientation = orientationResponse.text.trim().toLowerCase();
      let rotationAngle = 0;
